...
之前的一篇月报[MySQL · 源码分析 · 原子DDL的实现过程][1]对MySQL8.0的原子DDL的背景以及使用的一些关键数据结构进行了阐述，同时也以CREATE TABLE为例介绍了Server层和Storage层统一系统表后如何创建一张新表进行了介绍。接下来本篇文章，我们将以DROP TABLE为例来继续看一下MySQL8.0对于DDL执行成功和执行失败时，如何实现DDL事务的提交和回滚。  
...
[1]: http://mysql.taobao.org/monthly/2018/03/02/
[0]: http://ata2-img.cn-hangzhou.img-pub.aliyun-inc.com/a599cf1ef61639310dbf336d0192c7f8.png